...
function ApplyNocRequest() {
    ...
    useEffect(() => {
        const loadHsCodes = async () => {
            try {
                const res = await HsCodeOptionsDescription();
                setHsCodeOptions(res.HsCodeOptions);
                setHsCodeDescrip(res.HsCodeDescrip);
            } catch (error) {
                console.error("HS Code fetch failed", error);
            }
        };

        loadHsCodes();
    }, []);
    const [nature_of_biomaterialoptions, setNatureOfBiomaterialoptions] = useState([]);
    useEffect(() => {
        const loadNBioMat = async () => {
            try {
                const res = await natureOfBiomaterialOptions();
                setNatureOfBiomaterialoptions(res.nature_of_biomaterialoptions);
            } catch (error) {
                console.error("Natural Data fetch failed", error);
            }
        };

        loadNBioMat();
    }, []);
    const handleNatureChange = (selectedOptions) => {
        ...
        setFormData((prev) => ({
            ...prev,
            nature_of_biomaterial: values,
        }));
    };
    const [where_sample_collectedOption, setwhereSampleCollectedOption] = useState([]);
    useEffect(() => {
        const loadSampleCollected = async () => {
            try {
                const res = await whereSampleCollectedOption();
                setwhereSampleCollectedOption(res.where_sample_collectedOption);
            } catch (error) {
    ...
    useEffect(() => {
        const loadSampleCollected = async () => {
            try {
                const res = await selectSpecifyPurposeOption();
                setSpecifyPurposeOption(res.SpecifyPurposeOption);
            } catch (error) {
                console.error("Sample Data fetch failed", error);
            }
        };
    ...
    useEffect(() => {
        const loadSamplesUsedResear = async () => {
            try {
                const res = await funcwhetherSamplesUsedResearchOption();
                setwhetherSamplesUsedResearchOption(res.whetherSamplesUsedResearchOption);
            } catch (error) {
                console.error("Sample Data fetch failed", error);
            }
        };
        loadSamplesUsedResear();
    }, []);


    const [PurposeofSamplesOption, setPurposeofSamplesOption] = useState([]);
    useEffect(() => {
        const loadPurposeofSamples = async () => {
            try {
                const res = await funcPurposeofSamplesOption();
                setPurposeofSamplesOption(res.PurposeofSamplesOption);
            } catch (error) {
                console.error("Sample Data fetch failed", error);
            }
        };
        loadPurposeofSamples();
    }, []);
    ...
    useEffect(() => {
        const loadPurposeofSamples = async () => {
            try {
                const res = await quantityofSampleExportedOptions();
                setquantityofSampleExported(res.quantityofSampleExported);
            } catch (error) {
                console.error("Sample Data fetch failed", error);
            }
        };
    ...
    /* Checkbox handler */
    const handleNatureCheckboxChange = (value) => {
        setFormData((prev) => {
            const exists = prev.nature_of_biomaterial.includes(value);

            const updatedValues = exists
                ? prev.nature_of_biomaterial.filter((v) => v !== value)
                : [...prev.nature_of_biomaterial, value];

            setShowTextAreaNature(updatedValues.includes("Others"));

            return {
                ...prev,
                nature_of_biomaterial: updatedValues
            };
        });
    };

    /* Remove selected value */
    const removeNatureValue = (value) => {
        setFormData((prev) => {
            const updatedValues = prev.nature_of_biomaterial.filter(
                (item) => item !== value
            );
                ...
                <Container fluid>
                    <Row>
                        <Col lg={12} md={9} sm={12}>
                            <Col className="page-title-box mb-3">
                                <Col className="d-flex align-items-center justify-content-center position-relative">
                                    <h3 className="m-0" style={{ fontSize: "20px", fontWeight: "600", color: "#14468C" }}>Application form for Export of Human Biological Material</h3>
                                    <div className="position-absolute end-0">
                                        <Button className="d-flex" variant="primary"><FaLongArrowAltLeft className="mt-1" /> Back</Button>
                                    </div>
                                </Col>
                            </Col>
                        </Col>
                                        ...
                                        <>
                                            ...
                                            <Card.Body>
                                                ...
                                                <div className="d-flex justify-content-between mt-2 m-2">
                                                    ...
                                                    <Button className="d-flex" variant="primary" size="md" onClick={() => setStep(2)}>Add More Info <FaArrowRight className="mt-1" /></Button>
                                                </div>
                                            </Card.Body>
                                        </>
                                    )}
                                                ...
                                                <Row>
                                                    ...
                                                    <Col md={4}>
                                                        <Form.Group>
                                                            <Form.Label>
                                                                <strong>
                                                                    (ii) Nature of biomaterial to be exported
                                                                    <span className="text-danger">*</span>
                                                                </strong>
                                                            </Form.Label>
                                                            ...
                                                            <div className="dropdown">
                                                                <button
                                                                    ...
                                                                    aria-expanded="false"
                                                                >
                                                                    {formData.nature_of_biomaterial.length > 0 && (
                                                                        <div className="mt-2 d-flex flex-wrap gap-2">
                                                                            {formData.nature_of_biomaterial.map((value) => (
                                                                                <span
                                                                                    key={value}
                                                                                ...
                                                                                >
                                                                                    ...
                                                                                    <button
                                                                                        ...
                                                                                        aria-label="Remove"
                                                                                        onClick={() => removeNatureValue(value)}
                                                                                    ></button>
                                                                                </span>
                                                                            ))}
                                                                        </div>
                                                                        ...
                                                                        <li key={option.value} className="dropdown-item">
                                                                            <div className="form-check">
                                                                                <input
                                                                                    ...
                                                                                    type="checkbox"
                                                                                    id={`bio_${option.value}`}
                                                                                    checked={formData.nature_of_biomaterial.includes(option.value)}
                                                                                    onChange={() =>
                                                                                        handleNatureCheckboxChange(option.value)
                                                                                    }
                                                                                />
                                                                                <label
                                                                                    className="form-check-label"
                                                                                    htmlFor={`bio_${option.value}`}
                                                                                >
                                                                                    {option.label}
                                                                                </label>
                                                                            </div>
                                                            ...
                                                            {errors.nature_of_biomaterial && (
                                                                <p className="text-danger">{errors.nature_of_biomaterial}</p>
                                                            )}
                                                        </Form.Group>
                                                    ...
                                                    <Col md={6}>
                                                        ...
                                                        <Form.Group>
                                                            <Col>
                                                                <Form.Check inline type="radio" value="Yes" name="repeat_samples_envisaged_yesno" checked={formData.repeat_samples_envisaged_yesno === "Yes"} onChange={handleChange} label="Yes" />
                                                                <Form.Check inline type="radio" value="No" name="repeat_samples_envisaged_yesno" checked={formData.repeat_samples_envisaged_yesno === "No"} onChange={handleChange} label="No" />
                                            ...
                                            <Card.Body>
                                                <Row>
                                                    <Col as={Col} md="12">
                                                        ...
                                                        <Col>
                                                            <Form.Check inline type="radio" value="Yes" name="leftover_samples_store_yes_no" checked={formData.leftover_samples_store_yes_no === "Yes"} onChange={handleChange} label="Yes" />
                                                            <Form.Check inline type="radio" value="No" name="leftover_samples_store_yes_no" checked={formData.leftover_samples_store_yes_no === "No"} onChange={handleChange} label="No" />
                                                        ...
                                                        {showInputsLeftoverSamples && (
                                                            <>
                                                                <Row>
                                                                    <Form.Group as={Col} md="6" className="mb-3" controlId="sampleCollectedDescription">
                                                                        <Form.Label>
                                                                            <strong>Provide details
                                                                                <span className="text-danger">*</span></strong>
                                                                        </Form.Label>
                                                ...
                                                <Row>
                                                    <Col xl={12} className="d-flex justify-content-between mt-3">
                                                        <Col className="col-md">
                                                            <Button variant="warning" size="md" className="d-flex align-items-center" onClick={() => setStep(1)}><FaArrowLeft /> <span className='ps-2'>Back to Previous</span></Button>
                                                        </Col>
                                                        <Col className="col-md d-flex gap-1 justify-content-end">
                                                            <Button variant="primary" size="md" onClick={saveAsDraft}>Save as Draft</Button>
                                                            <Button variant="primary" className="d-flex align-items-center" size="md" onClick={() => setStep(3)}><span className='ps-2'> Add More Info</span><FaArrowRight /> </Button>
                                                        </Col>
                                                    </Col>
                                                </Row>
                                            </Card.Body>
                                            ...
                                            <Card.Body>
                                                <Row>
                                                    ...
                                                    <Form.Group as={Col} md="6">
                                                        ...
                                                        <Form.Control type="file" id="exampleFormControlFile1" name="certified_copy_proforma" value={formData.certified_copy_proforma} onChange={formData.certified_copy_proforma} />
                                                    </Form.Group>
                                                </Row>
                                            </Card.Body>
                                            <Col className="form-card-sub-tite text-black">
                                            ...
                                            <Card.Body>
                                                <Row>
                                                    <Form.Group as={Col} md="6">
                                                        ...
                                                        <Form.Control type="file" id="exampleFormControlFile1" name="declaration_letter" value={formData.declaration_letter} onChange={formData.declaration_letter} />
                                                    </Form.Group>
                                                    <Col md={6}>
                                                ...
                                                <Row>
                                                    <Col xl={12} className="d-flex justify-content-between mt-2">
                                                        <Col className="col-md">
                                                            <Button variant="warning" size="md" className="d-flex align-items-center" onClick={() => setStep(2)}><FaArrowLeft /> <span className='ps-2'>Back to Previous</span></Button>
                                                        </Col>
                                                        <Col className="col-md d-flex justify-content-end gap-1">
                                                            <Button variant="primary" size="md" onClick={saveAsDraft}>Save as Draft</Button>
                    ...
                    <Modal show={showModal} onHide={() => setShowModal(false)}>
                        ...
                        <Modal.Footer>
                            <Button color="primary" onClick={() => setShowModal(false)}>OK</Button>
                        </Modal.Footer>
                    </Modal>
                </Container>
            </div >
